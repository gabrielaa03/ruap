			...
			<form name="frmRegistration" method="post" action="">
				<table border="0" class="tableForChecking" >
					...
					<tr>
						...
						<td class="tableRow1"><input type="number" name="GlazingArea" step="0.01" value="<?php if(isset($_POST['GlazingArea'])) echo $_POST['GlazingArea']; ?>"></td>
		...
		{ 	
			...
			if($relativeCompactness && $surfaceArea && $wallArea  && $roofArea && $overallHeight && $orientation && $glazingArea  && $glazingAreaDistribution){
					...
					if ($radioButton == "heatingLoad") { 
						...
						curl_setopt($ch2, CURLOPT_HTTPHEADER, array('Content-Type: application/json', 'Authorization: Bearer '.$api_key2, 'Accept: application/json'));
						...
						curl_close($ch);
						$result2 =$response2["Results"]["output1"]["value"]["Values"][0][10];	
						$result3 = $result2*100;
						echo round($result3, 2);echo "%";
						?> 
						 </div> 
						<?php
						session_start();
